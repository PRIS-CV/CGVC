...
class PyramidFeatures(nn.Module):
    ...
    def __init__(self, B4_size, B5_size, feature_size=256):
        # B4_size = 1024, B5_size = 2048
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=196):
        ...
        self.fpn = PyramidFeatures(fpn_sizes[2], fpn_sizes[3])
        ...
        self.classifier3 = nn.Sequential(
            nn.Linear(self.num_ftr3, 9),
        )
        self.classifier4 = nn.Sequential(
            nn.Linear(self.num_ftr4, 196),
        )
        self.classifier5 = nn.Sequential(
            nn.Linear(hidden_number, 196),
        )
    ...
    def forward(self, x, targets):
        ...
        order_output = self.classifier3(f3)
        ...
        ce_loss = ce_loss_order + ce_loss_spec + ce_loss_species

        return ce_loss, [species_output, targets], [order_output, order_targets]
...
def train(epoch, net, trainloader, optimizer):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        ce_loss, [species_output, species_targets], [order_output, order_targets] = net(inputs, targets)
        loss = ce_loss
        loss.backward()
        optimizer.step()
    ...
    print('Epoch %d, train_order_acc = %.5f, train_species_acc = %.5f, '
          'train_loss = %.6f' % (epoch, train_order_acc, train_species_acc, train_loss))
...
def test(epoch, net, testloader):
    ...
    print('Epoch %d, test_order_acc = %.5f, test_species_acc = %.5f, '
          'test_loss = %.6f' % (epoch, test_order_acc, test_species_acc, test_loss))
    return test_order_acc, test_species_acc, test_loss
...
def main():
    ...
    trainset = torchvision.datasets.ImageFolder(root=r'/data/chenjunhan/Cross-Granularity/Cars/train', transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=BATCH_SIZE, shuffle=True, num_workers=16,
                                              drop_last=True)

    testset = torchvision.datasets.ImageFolder(root=r'/data/chenjunhan/Cross-Granularity/Cars/test', transform=transform_test)
